...
- **Search:** Four retrieval options — `search_corpus()` (regex/KWIC), `search_index()` (BM25), `search_vector()` (cosine over your own embeddings), `search_dict()` (dictionary match; replaces `ner_extract_entities`).
- Common parameters standardized: `corpus` (replaces `tif`), `by` (replaces `text_hierarchy`).

## Removed

- In-package embedding generation (e.g. Hugging Face API). Use your own embedding pipeline and pass the matrix into `search_vector()` / `search_neighbors()`.
- Legacy names: `web_search`, `wiki_search`, `wiki_find_references`, `web_scrape_urls`, `ner_extract_entities`, `sem_nearest_neighbors` / `sem_search_corpus` (replaced by `search_vector` and `search_corpus`).
