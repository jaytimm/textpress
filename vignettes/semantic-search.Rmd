...
run_api <- nzchar(Sys.getenv("HUGGINGFACE_API_TOKEN"))
run_llm <- run_api && requireNamespace("ellmer", quietly = TRUE) && nzchar(Sys.getenv("OPENAI_API_KEY"))
```
...
```{r search-semantic, eval=run_api}
...
```{r rrf, eval=run_api, message=FALSE, warning=FALSE}
k <- 60
