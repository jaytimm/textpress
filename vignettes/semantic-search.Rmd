...
Embed the chunk text (with context) via a Hugging Face inference endpoint. The result is a numeric matrix with one row per chunk; use it with `search_vector()` for semantic search. The vignette uses `HUGGINGFACE_API_TOKEN` set in the setup chunk.
...
Context and chat are set once; each example below uses them. Requires `ellmer`. The vignette uses `OPENAI_API_KEY` set in the setup chunk.
...
chat <- ellmer::chat_openai(model = "gpt-4o-mini", credentials = function() OPENAI_API_KEY)
```
