...
age_range = "\\b(?:ages?|between|under\\s+age|over\\s+age|aged\\s+between)\\s+\\d{2,3}(?:\\s*(?:[-–]|to)\\s*\\d{2,3})?(?:\\s+and\\s+(?:older|younger|over|under))?\\b|\\b\\d{2,3}(?=\\s*\\+|\\s+and\\s+(?:older|younger|over|under))\\b",
